...
export class ImageHandler {
  ...
  private async applyResize(originalImage: sharp.Sharp, edits: ImageEdits): Promise<void> {
    ...
    if (edits.aspectRatio && edits.resize.width && !edits.resize.height) {
      const ratio = this.parseAspectRatio(edits.aspectRatio);
      if (ratio) {
        edits.resize.height = Math.round(Number(edits.resize.width) / ratio);
      }
    }
    ...
    if (edits.focalX !== undefined && edits.focalY !== undefined && resize.width && resize.height) {
      ...
      const targetW = Number(resize.width);
      const targetH = Number(resize.height);
      ...
      if (origAR > targetAR) {
        ...
        cropH = origH;
        cropW = Math.round(origH * targetAR);
        left = Math.round(origW * Number(edits.focalX) - cropW / 2);
        left = Math.max(0, Math.min(origW - cropW, left));
        top = 0;
      } else {
        // Original is taller than target - crop vertically
        cropW = origW;
        cropH = Math.round(origW / targetAR);
        top = Math.round(origH * Number(edits.focalY) - cropH / 2);
        top = Math.max(0, Math.min(origH - cropH, top));
        left = 0;
      }

      originalImage.extract({ left, top, width: cropW, height: cropH });
   ...
   * @param aspectRatio Aspect ratio string (e.g. "16:9" or "1.5")
   * @returns Parsed aspect ratio or null
   */
  private parseAspectRatio(aspectRatio: string): number | null {
